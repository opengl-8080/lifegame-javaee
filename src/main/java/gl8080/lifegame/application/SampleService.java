...
public class SampleService {
    ...
    public void searchDef(long id) {
        System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
        this.gameDefinitionRepository.search(id).ifPresent(def -> {
            def.getCellDefinitions().forEach((p, c) -> {
    ...
    public void registerGame(int gameDefId) {
        this.gameDefinitionRepository.search(gameDefId).ifPresent(gameDef -> {
            Game game = new Game(gameDef);
            this.gameRepository.register(game);
            logger.info("register game");
        });
    }
